...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 flex h-14 items-center justify-between gap-6 border-b border-zinc-800 bg-card px-4 md:px-6">
      {/* Logo */}
      <Link
        href="/"
        ...
        {NAV_LINKS.map(({ href, label }) => (
          <Link
            ...
            className={cn("text-sm font-normal text-white transition-opacity hover:opacity-80",
              pathname === href && "text-accent underline")}
          >
            {label}
          </Link>
        ))}
      ...
      {/* Search */}
      <div className="hidden flex-1 md:block max-w-[186px]">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="h-9 w-full rounded-md border border-zinc-700 bg-zinc-800/80 pl-9 pr-3 text-sm text-[#DEDEDE] placeholder:text-zinc-500 focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent"
          />
        </div>
      </div>
