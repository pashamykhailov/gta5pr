...
export function TransfersWidget({
  ...
  return (
    <div>
      <span className="text-sm font-semibold text-subheading">Топ 30 трансферов</span>
      <article className="rounded-lg border border-zinc-700 bg-card p-2">
          ...
          {transfers.map((transfer, index) => (
            ...
            >
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-zinc-700">
                <svg
                  className="h-5 w-5 text-subheading"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden
                >
              ...
              <div className="flex w-full grow justify-between max-w-[100px]">
                <EmblemPlaceholder />
                <div className="text-subheading self-center" aria-hidden>
                  <svg
                    className="h-4 w-4"
                    fill="none"
                    stroke="currentColor"
        ...
        {viewAllHref && (
          <Link
            href={viewAllHref}
            className="mt-1 block text-center text-xs text-subheading transition-colors hover:text-accent"
          >
            {viewAllText}
          </Link>
        )}
